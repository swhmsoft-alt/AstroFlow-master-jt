...
print(f'{len(keys)} grade FAQ keys')
...
for lang in langs:
    ...
    if not needs:
        ...
        continue
    print(f'{lang} ({lang_names[lang]}): translating {len(needs)} keys...')
    body = json.dumps({
        'model': 'deepseek-chat',
        'messages': [
            {'role': 'system', 'content': f'Translate JSON values from English to {lang_names[lang]}. Keep keys. Return valid JSON.'},
