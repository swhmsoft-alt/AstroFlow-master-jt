...
keys = ['equipment.list.hero.title', 'equipment.list.hero.highlight']
en = json.load(open(td + '/en.json', 'r', encoding='utf-8'))
source = {k: en[k] for k in keys}

for lang, lname in [('de','German'),('ja','Japanese'),('fr','French'),('es','Spanish'),
    ...
    body = json.dumps({
        'model': 'deepseek-chat',
        'messages': [
            {'role': 'system', 'content': f'Translate JSON from English to {lname}. These are section headings for a CNC manufacturing website equipment page. Keep keys. Valid JSON only. Short and concise.'},
