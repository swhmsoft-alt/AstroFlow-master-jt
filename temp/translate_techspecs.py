...
keys = [k for k in en if k.startswith('materials.techspecs.')]
source = {k: en[k] for k in keys}
print(f'Found {len(keys)} techspecs keys')
...
for lang in langs:
    ...
    body = json.dumps({
        'model': 'deepseek-chat',
        'messages': [
            {'role': 'system', 'content': f'You are a professional {lang_names[lang]} translator. Translate the following JSON values from English to {lang_names[lang]}. Keep keys unchanged. Return ONLY valid JSON.'},
