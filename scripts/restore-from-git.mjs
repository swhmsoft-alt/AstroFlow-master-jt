/**
 * Restore all source files from git HEAD to undo the bad translations.
 */
...
for (const file of files) {
  ...
  try {
    ...
    // Try with forward slashes for git
    try {
      const altFile = file.replace(/\\/g, '/');
      const result = execSync(`git show HEAD:${altFile}`, { 
        encoding: 'utf-8', 
        timeout: 10000,
        stdio: ['pipe', 'pipe', 'pipe']
      });
      ...
      console.log(`  ✗ ${file}: ${err2.message.substring(0, 80)}`);
      failed++;
    }
  }
}
