...
const startMarker = '<!-- ── RICH BLUEPRINT BODY';
...
const richSection = `  <!-- ── RICH BLUEPRINT BODY (Section 1-3) ── -->
  {data.sku ? (
        ...
        <div class="mt-6 flex flex-wrap gap-4 text-sm">
          {data.material && (
            <div class="flex items-center gap-2 px-4 py-2 rounded-lg border" style="background: var(--theme-bg); border-color: color-mix(in srgb, var(--theme-primary) 8%, transparent);">
              ...
              <a href={'/products/materials/' + (material?.slug || material?.id?.replace('.json','') || '') + '/'} class="font-semibold" style="color: var(--theme-primary);">{data.material.replace(/\\(.*\\)/, '').trim()}</a>
            </div>
          )}
          {data.industry && (
            <div class="flex items-center gap-2 px-4 py-2 rounded-lg border" style="background: var(--theme-bg); border-color: color-mix(in srgb, var(--theme-primary) 8%, transparent);">
...
writeFileSync(templatePath, header + richSection + footer, 'utf-8');
