...
const keys = Object.keys(en).filter(k => k.startsWith('materials.standards.badges.') || k.startsWith('materials.standards.subtitles.'));
const source = {};
...
async function translate(lang) {
  ...
  const body = JSON.stringify({
    model: 'deepseek-chat',
    messages: [
      { role: 'system', content: `You are a professional ${langNames[lang]} translator for industrial content. Translate JSON values from English to ${langNames[lang]}. Keep keys. Return ONLY valid JSON.` },
...
(async () => { for (const lang of langs) await translate(lang); console.log('Done!'); })();
