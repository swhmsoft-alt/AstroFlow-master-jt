/**
 * Translate reactiveweldingknowhow keys for all 8 target languages using DeepSeek API.
 * Step 1 (JA) already done manually. This script handles de, fr, es, pt, it, ko, nl, pl.
 */
import fs from 'fs';
...
const PREFIX = 'services.reactiveweldingknowhow.';

const en = JSON.parse(fs.readFileSync(EN_JSON, 'utf-8'));

const relevantKeys = Object.keys(en).filter(k => k.startsWith(PREFIX));

console.log(`Found ${relevantKeys.length} reactiveweldingknowhow keys in en.json`);
...
RULES:
- Keep technical terms/acronyms unchanged: TIG, CNC, LPM, HAZ, AWS, GTAW, GMAW, FEA
- Keep measurement units and values unchanged: "15-20 LPM", "200-300 mm", "250°C", "540-650°C", "8.6 µm/m·K", "114 GPa"
- Keep ALL special characters, symbols: "—", "×", "±", "≥", "≤", "°", "&", "<", ">", "µm"
