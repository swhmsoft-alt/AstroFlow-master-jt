/**
 * Translate FabProcessSpectrum keys for all 8 target languages using DeepSeek API.
 */
import fs from 'fs';
...
const PREFIX = 'services.fabprocessspectrum.';

const en = JSON.parse(fs.readFileSync(EN_JSON, 'utf-8'));

const relevantKeys = Object.keys(en).filter(k => k.startsWith(PREFIX));

console.log(`Found ${relevantKeys.length} fabprocessspectrum keys in en.json`);
...
RULES:
- Keep technical terms/acronyms unchanged: TIG, CNC, LPM, O₂
- Keep measurement units and values unchanged: "15-25 LPM", "<50 ppm", "2\" to 24\"", "10-80"
