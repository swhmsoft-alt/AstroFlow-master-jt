...
missing = {k: en[k] for k in sorted(en) if k.startswith('cap.inspectionpage.') and k not in data}
...
for i in range(0, len(keys), BATCH):
    ...
    payload = json.dumps({
        'model': 'deepseek-chat',
        'messages': [
            {'role': 'system', 'content': 'Professional Korean translator for industrial/manufacturing content. Keep CMM, OES, SPC, FPI, UT, NDT, MTR, CoC, FAIR, AS9102, ASME, ASTM, ISO, EN, Nadcap, GD&T, ZEISS, CONTURA, OGP, SPECTROMAXx, Mitutoyo exactly as-is. Return ONLY valid JSON.'},
...
print(f'✅ ko.json saved ({len(sorted_data)} total keys)')
